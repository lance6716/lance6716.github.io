...
TLC 只能处理限定量词`\E x \in S: p`的表达式，而且`S`必须是有限集。与 TLA+ 不同，TLC 不能处理不绑定量词的表达式`\E x: p`。

TLC 在`CHOOSE`无法取到合法的值的时候会报错。

TLC 仅在需要的时候求值，例如`[n \in Nat |-> n * (n + 1)][3]`是可以求值的。
...
TLC 可以对满足如下条件的 TLA+ 时序逻辑公式求值：表达式良定义并且其组成部分都可以求值。良定义指的是表达式由如下四种公式合取而成：
...
- 由如下两种合取而成的公式
  ...
  - 满足公平性、`[]<><<A>>_v`、`<>[][A]_v`的形式，其中`A`是动作`v`是单状态公式。公平性的组成部分是`<<A>>_v`和`ENABLED <<A>>_v`
...
在模型检查模式中，`G`中的节点实际上是*视图*而不是状态，这么做的原因是*视图*可以控制使用部分变量区分`G`的点。*视图*在配置文件的`VIEW`进行指定。*视图*常见的应用是，用户为了调试规范增加了一些辅助变量，但是这样会增大搜索空间，所以用*视图*指定为原始的非调试变量。
